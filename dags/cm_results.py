...
with DAG(
    ...
    cm_results = SimpleHttpOperator(
        ...
        response_check=lambda response: True if (
            'status' in response.json()) else False,
        log_response=True,
        dag=dag)
    ...
    def sanitize(data):
        ...
        htmlFilter.feed(data)
        return ''.join(i for i in htmlFilter.text if not i in bad_chars)
    ...
    def python_parse_results(**kwargs):
        ...
        if(results['next_finished_after_timestamp']):
            Variable.set("CM_RESULTS_TIMESTAMP",
                         results['next_finished_after_timestamp'])
